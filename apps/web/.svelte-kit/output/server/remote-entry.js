...
import { C as normalize_issue, D as MUTATIVE_METHODS, S as flatten_issues, T as throw_on_old_property_access, b as deep_set, s as handle_error_and_jsonify, w as set_nested_value, y as create_field_proxy } from "./chunks/utils.js";
...
function form(validate_or_fn, maybe_fn) {
	...
	function create_instance(key) {
		...
		const __ = {
			...
			fn: async (data, meta, form_data) => {
				...
				if (validated?.issues !== void 0) handle_issues(output, validated.issues, form_data);
					...
					try {
						output.result = await run_remote_function(event, state, true, () => data, (data$1) => !maybe_fn ? fn() : fn(data$1, issue));
					} catch (e) {
		...
		if (key == void 0) Object.defineProperty(instance, "for", { value: (key$1) => {
			const { state } = get_request_store();
			const cache_key = __.id + "|" + JSON.stringify(key$1);
			let instance$1 = (state.remote.forms ??= /* @__PURE__ */ new Map()).get(cache_key);
			if (!instance$1) {
				instance$1 = create_instance(key$1);
				instance$1.__.id = `${__.id}/${encodeURIComponent(JSON.stringify(key$1))}`;
				instance$1.__.action_id = `${__.id}/${JSON.stringify(key$1)}`;
				instance$1.__.name = __.name;
				state.remote.forms.set(cache_key, instance$1);
			}
			return instance$1;
		} });
		return instance;
...
function prerender(validate_or_fn, fn_or_options, maybe_options) {
	...
	const wrapper = (arg) => {
		...
		const promise = get_response(__, payload, state, async () => {
			...
			const promise$1 = run_remote_function(event, state, false, () => validate(arg), fn);
			if (state.prerendering) state.prerendering.remote_responses.set(url, promise$1);
	...
	fail(error$1) {
		...
		for (const subscriber of this.#subscribers) {
			...
			if (subscriber.waiting_reject) {
				...
				subscriber.waiting_reject = null;
				reject(error$1);
			} else subscriber.pending_error = { error: error$1 };
		}
		this.#subscribers.clear();
	}
	subscribe(options) {
		...
		const iterator = {
			next() {
				if (subscriber.pending_error) {
					const { error: error$1 } = subscriber.pending_error;
			...
			setTimeout(async () => {
				...
				try {
					return await run_remote_function(event, {
						...state,
						is_in_remote_query: true
					}, false, async () => Promise.all(entries.map((entry$1) => entry$1.get_validated())), async (input) => {
						...
						for (let i = 0; i < entries.length; i++) try {
							...
							for (const resolver of entries[i].resolvers) resolver.reject(error$1);
						}
					});
				} catch (error$1) {
					for (const entry$1 of batched.values()) for (const resolver of entry$1.resolvers) resolver.reject(error$1);
				}
			}, 0);
		});
	};
			...
			return run_remote_function(event, {
				...
				return Promise.all(input.map(async (arg, i) => {
					try {
						...
						return {
							type: "error",
							error: await handle_error_and_jsonify(event, state, options, error$1),
	...
	return new SharedIterator((instance) => {
		...
		(async () => {
			try {
				...
				if (!aborted) instance.fail(error$1);
			} finally {
				close();
			}
		})();
...
function requested(query$1, limit) {
	const { event, state } = get_request_store();
	const internals = query$1.__;
	...
	const record_failure = (payload, error$1) => {
		const promise = Promise.reject(error$1);
		promise.catch(noop);
	...
	const result = {
		*[Symbol.iterator]() {
			for (const payload of selected) try {
				...
				record_failure(payload, error$1);
				continue;
			}
		},

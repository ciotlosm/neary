...
export const useStationStore = create<StationStore>((set, get) => ({
  ...
  // Actions
  loadStops: async (agency_id: number) => {
  ...
  loadStopsByLocation: async (lat: number, lon: number) => {
    set({ loading: true, error: null });
    
    try {
      // Import service dynamically to avoid circular dependencies
      const { stationService } = await import('../services/stationService');
      const stops = await stationService.getStopsByLocation(lat, lon);
      
      set({ stops, loading: false, error: null });
    } catch (error) {
      set({ 
        loading: false, 
        error: error instanceof Error ? error.message : 'Failed to load stops by location'
      });
    }
  },
